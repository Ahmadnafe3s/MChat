...
const Messages = memo(
    ({ data }: { data: Conversations }) => {
        ...
        const getMediaDimensions = (aspectRatio = 4 / 3) => {
            const calculatedWidth = Math.min(Math.max(minMediaWidth, maxMediaWidth), maxMediaWidth);
        ...
        const renderMedia = () => {
            ...
            if (format === "pdf" || format === "csv" || format === "xlsx" || format === "pptx") {
                return (
                    <View className="bg-gray-50 rounded-lg p-4 min-w-[250px]">
                        <View className="flex-row items-center mb-3">
                            <View className="w-10 h-10 bg-red-100 rounded-lg items-center justify-center mr-3">
                                <Text className="text-red-600 text-xs font-bold uppercase">{format}</Text>
                            </View>
                        ...
                        {isDownloading && (
                            <View className="mb-3">
                                <View className="flex-row justify-between items-center mb-1">
                                    ...
                                    <Text className="text-xs text-gray-600">{Math.round(downloadProgress)}%</Text>
                                </View>
                        ...
                        >
                            ...
                            <Text className={`font-medium text-sm ${isDownloading
                                ? 'text-gray-600'
                                : isSender
                                    ? 'text-white'
                                    : 'text-white'
        ...
        return (
            <>
                ...
                >
                    <View className={`flex max-w-[85%]  ${isSender ? 'items-end' : 'items-start'}`}>
                        {/* Message bubble */}
                        <View
                            className={`px-3 py-2 elevation ${isSender
