...
import { Dimensions, FlatList, Image, Keyboard, Modal, Pressable, StyleSheet, Text, TouchableOpacity, View } from 'react-native'
...
const QuickReplies = () => {
  ...
  const { quickReplies, createQuickReply } = useQuickReply({ afterSuccess: () => setForm(false) })
  const [selectedReply, setSelectedReply] = useState<QuickReply | null>(null)
  const [form, setForm] = useState(false)
  const { selectedChat } = useChatStore()
  const { sendMessage } = useChat();
  const { handleSubmit, control, formState: { errors } } = useForm<z.infer<typeof quickReplySchema>>({
    ...
    <>
      <SafeAreaView className='bg-white flex-1'>
        ...
        <View className='flex flex-row p-5 gap-4 mt-1 border-b border-gray-200 items-center justify-between'>
          ...
          <TouchableOpacity onPress={() => setForm(true)}>
            <Image source={icons.add as any} className="w-9 h-9" tintColor={"#42d6a6"} />
          </TouchableOpacity>
        </View>
        {/* Quick Reply List */}
        <FlatList
      ...
      <Modal visible={!!selectedReply} transparent={true} animationType="fade" statusBarTranslucent={true}>
        <Pressable className='relative flex-1 bg-black/20 justify-center items-center' onPress={() => setSelectedReply(null)}>
          ...
          <Pressable className='bg-white mx-5 p-5 rounded-xl items-center gap-3' onPress={(e) => e.stopPropagation()}>
      ...
      <Modal visible={form} transparent={true} animationType="fade" statusBarTranslucent={true} onRequestClose={() => setForm(false)}>
        <Pressable className='relative flex-1 bg-black/20 justify-center items-center' onPress={() => setForm(false)}>
          <KeyboardAvoidingView behavior="padding" keyboardVerticalOffset={20}>
            <Pressable className='bg-white p-5 rounded-xl items-center w-64' style={{ width: width - 50 }} onPress={(e) => e.stopPropagation()}>
              <Text className='text-neutral-700 text-2xl font-JakartaBold mt-4'>Add a quick reply</Text>
